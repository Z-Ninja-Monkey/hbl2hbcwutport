...
extern "C" int Menu_Main(void)
{
	...
	unsigned long long sysmenu = _SYSGetSystemApplicationTitleId((SYSTEM_APP_ID) 0);
	...
	OSDynLoad_FindExport(cmpt_handle, (OSDynLoad_ExportType) 0, "CMPTLaunchTitle", (void**)&CMPTLaunchTitle);
	OSDynLoad_FindExport(cmpt_handle, (OSDynLoad_ExportType) 0, "CMPTAcctSetScreenType", (void**)&CMPTAcctSetScreenType);
	OSDynLoad_FindExport(cmpt_handle, (OSDynLoad_ExportType) 0, "CMPTGetDataSize", (void**)&CMPTGetDataSize);
	OSDynLoad_FindExport(cmpt_handle, (OSDynLoad_ExportType) 0, "CMPTCheckScreenState", (void**)&CMPTCheckScreenState);
...
static bool doIdSelect()
{
	...
	OSScreenInit();
	screen_buf0_size = OSScreenGetBufferSizeEx((OSScreenID) 0);
	screen_buf1_size = OSScreenGetBufferSizeEx((OSScreenID) 1);
	screenBuffer = (uint8_t*)MEMBucket_alloc(screen_buf0_size+screen_buf1_size, 0x100);
	OSScreenSetBufferEx((OSScreenID) 0, (void*)(screenBuffer));
	OSScreenSetBufferEx((OSScreenID) 1, (void*)(screenBuffer + screen_buf0_size));
	OSScreenEnableEx((OSScreenID) 0, 1);
	...
	while(1)
	{
		...
		if( redraw )
		{
			OSScreenClearBufferEx((OSScreenID) 0, 0);
			OSScreenPutFontEx((OSScreenID) 0, 0, 0, verChar);
			// Starting position.
			int gamelist_y = 1;
			int i;
			for (i = 0; i < ListMax; ++i, gamelist_y++)
			{
				...
				OSScreenPutFontEx((OSScreenID) 0, gamelist_y, 0, printStr);
			}
			OSScreenFlipBuffersEx((OSScreenID) 0);
			redraw = 0;
		}
	}
	//save selection(OSScreenID)
...
static unsigned int getButtonsDown()
{
	...
	VPADStatus vpad;
	VPADRead((VPADChan) 0, &vpad, 1, &vpadError);
	if(vpadError == 0)
	...
	for(i = 0; i < 4; i++)
	{
		...
		if(WPADProbe((WPADChan) i, &controller_type) != 0)
			continue;
		KPADStatus kpadData;
		KPADRead((WPADChan) i, &kpadData, 1);
